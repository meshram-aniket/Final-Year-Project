...
const registerUser = asyncHandler( async (req, res) => {
    ...
    const {fullName, email, username, password, address, role, location, isActive, phone } = req.body
    //console.log("email: ", email);

    if (
        [fullName, email, username, password, address, role, location, isActive, phone].some((field) => field?.trim() === "")
    ) {
        throw new ApiError(400, "All fields are required")
    }

    const existedUser = await Worker.findOne({
        $or: [{ username }, { email }]
    })
    ...
    const createdUser = await Worker.findById(user._id).select(
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const { accessToken, refreshToken } = await
        ...
        const loggedInUser = await Worker.findById(user._id)
...
const updateUserCoordinates = asyncHandler( async (req, res) => {
    ...
    try {
        ...
        // Find the user by ID
        const user = await Worker.findByIdAndUpdate(user_Id);
        ...
        user.location.coordinates = 
             [latitude, longitude] // Assuming your model expects [longitude, latitude] format
        

        // Save the updated user
        await user.save();

        return res.status(200).json({ message: 'User coordinates updated successfully', user });
    } catch (error) {
...
const getWatchHistory = asyncHandler(async(req, res) => {
    const user = await Worker.aggregate([
        {
            $match: {
